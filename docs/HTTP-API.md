...
WeFlow 提供本地 HTTP API（已支持GET 和 POST请求），便于外部脚本或工具读取聊天记录、会话、联系人和导出的媒体文件；也支持在检测到新消息后通过固定 SSE 地址主动推送消息事件。
...
群聊里 `groupNickname` 会优先来自消息源数据里的群昵称；若源数据缺失，则回退为空或展示名。

---

## 3.1 获取缓存消息

缓存接口只读取 WeFlow 本地 SQLite 消息缓存库，不直接访问微信 WCDB 原库。首次使用或需要刷新时，先调用 `POST /api/v1/messages/sync` 触发后台同步。
...
| -------- | ------ | ---- | ----------------------------------------------------- |
| `talker` | string | 是   | 会话 ID。私聊通常是对方 `wxid`，群聊是 `xxx@chatroom` |
| `limit`  | number | 否   | 返回条数，默认 `100`，范围 `1~1000`                   |
| `before` | string | 否   | 游标时间戳，返回该时间之前的消息                      |
| `start`  | string | 否   | 开始时间，支持 `YYYYMMDD` 或时间戳                    |
| `end`    | string | 否   | 结束时间，支持 `YYYYMMDD` 或时间戳                    |
...
```bash
curl "http://127.0.0.1:5031/api/v1/messages-cache?talker=wxid_xxx&limit=100"
curl "http://127.0.0.1:5031/api/v1/messages-cache?talker=xxx@chatroom&before=1760000000"
```
...
```json
{
  ...
  "count": 100,
  "hasMore": true,
  "nextCursor": 1760000000,
  "sync": {
    "status": "idle",
    "lastSyncStartedAt": 1760000123000,
    "lastSyncFinishedAt": 1760000125000,
    "cacheLagMs": 30000
  },
  "items": []
}
...
单条缓存消息字段包含：`localId`、`serverId`、`localType`、`createTime`、`sortSeq`、`isSend`、`senderUsername`、`content`、`rawContent`、`parsedContent`。群聊缓存同步时 `senderUsername` 继续置为空字符串。

---

## 3.2 触发消息缓存同步

同步接口只创建后台任务并立即返回，不会等待完整同步完成。同一时间只执行一个消息同步任务，单批同步上限为 `500` 条。
...
| 参数     | 类型   | 必填 | 说明                               |
| -------- | ------ | ---- | ---------------------------------- |
| `talker` | string | 是   | 会话 ID                            |
| `limit`  | number | 否   | 单批同步条数，默认 `500`，最大 `500` |
| `start`  | string | 否   | 同步开始时间，支持 `YYYYMMDD` 或时间戳 |
| `end`    | string | 否   | 同步结束时间，支持 `YYYYMMDD` 或时间戳 |
...
```json
{
  ...
  "job": {
    ...
    "talker": "wxid_xxx",
    "status": "completed",
    "startedAt": 1760000123000,
    "finishedAt": 1760000125000,
    "scannedCount": 500,
    "insertedCount": 480,
    "updatedCount": 20,
    "error": null
...
```json
{
  ...
  "state": {
    "talker": "wxid_xxx",
    "lastCreateTime": 1760000000,
    "lastSequence": 1760000000000,
    "syncStatus": "idle",
    "lastSyncFinishedAt": 1760000125000
  },
  "latestJob": null
...
4. 媒体访问链接只有在对应消息已经通过 `media=1` 导出后才可访问。
